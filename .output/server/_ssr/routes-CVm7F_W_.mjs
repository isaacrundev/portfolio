import { o as __toESM } from "../_runtime.mjs";
import { E as require_react, u as require_jsx_runtime } from "../_libs/@tanstack/react-router+[...].mjs";
import { n as Trans, t as useTranslation } from "../_libs/react-i18next.mjs";
import { a as ArrowUpFromLine, i as CodeXml, n as KeyRound, r as ExternalLink, t as UserRound } from "../_libs/lucide-react.mjs";
import { i as AnimatePresence, n as useScroll, r as motion, t as useTransform } from "../_libs/framer-motion.mjs";
import { t as m } from "../_libs/react-type-animation.mjs";
import { t as wrap } from "../_libs/popmotion.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/routes-CVm7F_W_.js
var import_react = /* @__PURE__ */ __toESM(require_react());
var import_jsx_runtime = require_jsx_runtime();
var BackToTop = () => {
	const handleClick = () => {
		window.scrollTo({
			top: 0,
			behavior: "smooth"
		});
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
		type: "button",
		onClick: handleClick,
		className: "fixed flex items-center justify-center text-brand-cyan transition-all duration-500 ease-in-out bg-slate-900/60 backdrop-blur-xl border border-brand-cyan/30 rounded-full shadow-[0_0_20px_rgba(6,182,212,0.3)] hover:shadow-[0_0_30px_rgba(6,182,212,0.6)] hover:bg-brand-cyan/10 hover:border-brand-cyan hover:scale-110 w-12 h-12 sm:w-14 sm:h-14 bottom-6 right-6 group z-50 pointer-events-auto overflow-hidden",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute inset-0 bg-brand-cyan/20 translate-y-[100%] group-hover:translate-y-0 transition-transform duration-500 ease-out" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ArrowUpFromLine, {
			className: "group-hover:-translate-y-1 relative z-10 transition-transform",
			strokeWidth: 2.5,
			size: 22
		})]
	});
};
var Header = ({ children }) => {
	const { scrollY } = useScroll();
	const opacity = useTransform(scrollY, [0, 50], [0, 1]);
	const y = useTransform(scrollY, [0, 50], [0, 0]);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.header, {
		id: "header",
		className: "fixed top-0 left-0 w-full z-50 py-4 px-6 md:px-12 pointer-events-none",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
			style: {
				opacity,
				y
			},
			className: "absolute inset-0 bg-slate-900/60 backdrop-blur-3xl border-b border-white/5 pointer-events-none -z-10"
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "max-w-7xl mx-auto flex justify-between items-center bg-transparent pointer-events-auto",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
				initial: {
					opacity: 0,
					x: -20
				},
				animate: {
					opacity: 1,
					x: 0
				},
				transition: {
					duration: .8,
					ease: "easeOut"
				},
				className: "text-2xl font-extrabold tracking-tighter",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "text-white hover:text-brand-cyan transition-colors cursor-pointer select-none",
					children: "I.W"
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.nav, {
				initial: {
					opacity: 0,
					y: -20
				},
				animate: {
					opacity: 1,
					y: 0
				},
				transition: {
					duration: .8,
					delay: .1,
					ease: "easeOut"
				},
				className: "flex flex-row gap-2 sm:gap-6 items-center p-2 rounded-full glass-card pointer-events-auto",
				children
			})]
		})]
	});
};
function MenuButton({ text, linkTo }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("a", {
		href: linkTo,
		className: "relative px-3 py-1.5 sm:px-4 sm:py-2 text-sm sm:text-base font-semibold text-slate-300 hover:text-white transition-all group rounded-full hover:bg-white/10 overflow-hidden",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			className: "relative z-10",
			children: text
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-full bg-brand-cyan/20 blur-md transition-all duration-300 group-hover:w-full z-0" })]
	});
}
var LanguageSwitcher = () => {
	const { i18n } = useTranslation();
	const toggleLanguage = () => {
		const newLang = (i18n.language?.startsWith("zh") ? "zh-TW" : "en") === "en" ? "zh-TW" : "en";
		i18n.changeLanguage(newLang);
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.button, {
		onClick: toggleLanguage,
		whileHover: { scale: 1.05 },
		whileTap: { scale: .95 },
		className: "ml-1 sm:ml-2 flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-white/5 border border-white/10 hover:bg-white/10 text-slate-200 text-sm sm:text-base transition-colors pointer-events-auto",
		title: i18n.language?.startsWith("zh") ? "Switch to English" : "切換至中文",
		children: i18n.language?.startsWith("zh") ? "EN" : "中"
	});
};
var dod1_default = "/assets/dod1-WdJTRfAl.png";
var dod2_default = "/assets/dod2-WX6KGgsR.png";
var dod3_default = "/assets/dod3-D-Fs-ivw.png";
var dod4_default = "/assets/dod4-CqewDsc4.png";
var cardGame1_default = "/assets/cardGame1-cdb6cKA4.png";
var cardGame2_default = "/assets/cardGame2-Z87eKbT8.png";
var cardGame3_default = "/assets/cardGame3-BDdrh6WP.png";
var cardGame_mobile1_default = "/assets/cardGame-mobile1-Dp02_E8p.png";
var cardGame_mobile2_default = "/assets/cardGame-mobile2-CoHCge91.png";
var cardGame_mobile3_default = "/assets/cardGame-mobile3-Dg6GbFUc.png";
var whistlerApp1_default = "/assets/whistlerApp1-BtHac4bb.png";
var whistlerApp2_default = "/assets/whistlerApp2-YxQyETJG.png";
var whistlerApp3_default = "/assets/whistlerApp3-BwRKIDF3.png";
var whistlerApp_mobile1_default = "/assets/whistlerApp-mobile1-CPinA0GK.png";
var whistlerApp_mobile2_default = "/assets/whistlerApp-mobile2-DNNdUfF6.png";
var whistlerApp_mobile3_default = "/assets/whistlerApp-mobile3-C3SpGP84.png";
var foxBook1_default = "/assets/foxBook1-R6nyEk2X.png";
var foxBook2_default = "/assets/foxBook2-CpGqTEB_.png";
var foxBook3_default = "/assets/foxBook3-DdCgJiq_.png";
var foxBook_mobile1_default = "/assets/foxBook-mobile1-CHvVr3ar.png";
var foxBook_mobile2_default = "/assets/foxBook-mobile2-Cph_3s8d.png";
var foxBook_mobile3_default = "/assets/foxBook-mobile3-DyAQ49AK.png";
var iconmonstr_email_10_96_default = "data:image/png;base64,";
var iconmonstr_github_1_96_default = "data:image/png;base64,";
var iconmonstr_linkedin_4_96_default = "data:image/png;base64,";
var _14C60149_3736_4BE8_86D4_AAF1BDA7F2AF_1_201_a_default = "/assets/14C60149-3736-4BE8-86D4-AAF1BDA7F2AF_1_201_a-e_jSj7GJ.jpeg";
var youbike_desktop1_default = "/assets/youbike-desktop1-C0fMXB6J.png";
var youbike_mobile1_default = "/assets/youbike-mobile1-DscWBeFf.png";
var mvdbScreenshots = [
	"/assets/mvdb_mobile_1-CXgbcNWg.png",
	"/assets/mvdb_mobile_2-B5Qt5YII.png",
	"/assets/mvdb_mobile_3-doIhfJOD.png",
	"/assets/mvdb_desktop_1-BrNsSMcs.png",
	"/assets/mvdb_desktop_2-DMuKbGdp.png",
	"/assets/mvdb_desktop_3-B0eqVJH6.png"
];
var youbikeScreenshots = [youbike_mobile1_default, youbike_desktop1_default];
var dodScreenshots = [
	dod1_default,
	dod2_default,
	dod3_default,
	dod4_default
];
var cardGameScreenshots = [
	cardGame1_default,
	cardGame2_default,
	cardGame3_default,
	cardGame_mobile1_default,
	cardGame_mobile2_default,
	cardGame_mobile3_default
];
var whistlerAppScreenshots = [
	whistlerApp1_default,
	whistlerApp2_default,
	whistlerApp3_default,
	whistlerApp_mobile1_default,
	whistlerApp_mobile2_default,
	whistlerApp_mobile3_default
];
var foxBookScreenshots = [
	foxBook1_default,
	foxBook2_default,
	foxBook3_default,
	foxBook_mobile1_default,
	foxBook_mobile2_default,
	foxBook_mobile3_default
];
var contactMethods = [
	{
		name: "Email",
		link: "https://formsubmit.co/el/dobuye",
		icon: iconmonstr_email_10_96_default,
		color: "group-hover:text-red-400 group-hover:glow-pink",
		border: "group-hover:border-red-500/50",
		bg: "group-hover:bg-red-500/10"
	},
	{
		name: "LinkedIn",
		link: "https://www.linkedin.com/in/isaac-842886255/",
		icon: iconmonstr_linkedin_4_96_default,
		color: "group-hover:text-blue-400 group-hover:glow-cyan",
		border: "group-hover:border-blue-500/50",
		bg: "group-hover:bg-blue-500/10"
	},
	{
		name: "GitHub",
		link: "https://github.com/isaacrundev",
		icon: iconmonstr_github_1_96_default,
		color: "group-hover:text-purple-400 group-hover:glow-purple",
		border: "group-hover:border-purple-500/50",
		bg: "group-hover:bg-purple-500/10"
	}
];
var Contact = () => {
	const { t } = useTranslation();
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		id: "contact",
		className: "py-20 relative flex justify-center overflow-hidden scroll-mt-28",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute bottom-0 w-full h-[600px] bg-gradient-to-t from-brand-purple/10 via-brand-pink/5 to-transparent -z-10" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute bottom-[-200px] left-1/2 -translate-x-1/2 w-[800px] h-[400px] bg-brand-cyan/20 blur-[150px] rounded-[100%] pointer-events-none -z-10" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex flex-col items-center w-full max-w-5xl px-8",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					initial: {
						opacity: 0,
						y: 30
					},
					whileInView: {
						opacity: 1,
						y: 0
					},
					transition: { duration: .8 },
					viewport: { once: true },
					className: "text-center mb-10",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
						className: "text-5xl md:text-7xl font-black tracking-tight text-white mb-6",
						children: [t("contact.title1"), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "text-gradient",
							children: t("contact.title2")
						})]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
						className: "text-xl md:text-2xl font-light text-slate-300 max-w-2xl mx-auto",
						children: t("contact.desc")
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					initial: "hidden",
					whileInView: "visible",
					variants: {
						hidden: {
							opacity: 0,
							scale: .95
						},
						visible: {
							opacity: 1,
							scale: 1
						}
					},
					transition: { duration: .8 },
					viewport: { once: true },
					className: "flex flex-col w-full glass-card rounded-[3rem] p-8 md:p-12 relative overflow-hidden",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full blur-[60px]" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						className: "grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 w-full z-10 relative",
						children: contactMethods.map((each, idx) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.a, {
							href: each.link,
							target: "_blank",
							rel: "noreferrer",
							whileHover: { y: -10 },
							whileTap: { scale: .95 },
							initial: {
								opacity: 0,
								y: 30
							},
							whileInView: {
								opacity: 1,
								y: 0
							},
							transition: {
								duration: .5,
								delay: idx * .15
							},
							viewport: { once: true },
							className: `group flex flex-col items-center justify-center gap-6 bg-slate-900/40 p-10 rounded-[2rem] border border-white/10 transition-all duration-500 backdrop-blur-md ${each.border} ${each.bg} shadow-[0_10px_30px_rgba(0,0,0,0.5)] hover:shadow-2xl`,
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
								className: "w-20 h-20 flex items-center justify-center bg-slate-950 rounded-2xl p-4 border border-white/5 group-hover:scale-110 transition-transform duration-500 shadow-inner",
								children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
									src: each.icon,
									alt: each.name,
									className: "w-full h-full object-contain filter drop-shadow-lg"
								})
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: `text-xl font-bold tracking-wide text-slate-300 transition-colors duration-500 ${each.color}`,
								children: each.name
							})]
						}, each.name))
					})]
				})]
			})
		]
	});
};
var containerVariants$1 = {
	hidden: { opacity: 0 },
	visible: {
		opacity: 1,
		transition: {
			staggerChildren: .15,
			delayChildren: .2
		}
	}
};
var itemVariants$1 = {
	hidden: {
		opacity: 0,
		y: 40,
		filter: "blur(10px)"
	},
	visible: {
		opacity: 1,
		y: 0,
		filter: "blur(0px)",
		transition: {
			duration: .8,
			ease: "easeOut"
		}
	}
};
var Hero = () => {
	const { t, i18n } = useTranslation();
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		id: "main",
		className: "flex flex-col min-h-screen pt-28 pb-12 justify-center items-center overflow-hidden relative",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "absolute inset-0 w-full h-full overflow-hidden -z-20 pointer-events-none",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute top-[20%] left-[10%] w-[500px] h-[500px] bg-brand-purple/20 rounded-full mix-blend-screen filter blur-[100px] animate-[blob_10s_infinite]" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute top-[30%] right-[10%] w-[400px] h-[400px] bg-brand-cyan/20 rounded-full mix-blend-screen filter blur-[100px] animate-[blob_12s_infinite_4s]" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute bottom-[10%] left-[40%] w-[600px] h-[600px] bg-brand-pink/10 rounded-full mix-blend-screen filter blur-[120px] animate-[blob_15s_infinite_2s]" })
			]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
			variants: containerVariants$1,
			initial: "hidden",
			whileInView: "visible",
			viewport: { once: true },
			className: "flex flex-col items-center justify-center text-center gap-6 px-6 sm:px-12 z-10 w-full max-w-5xl",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					variants: itemVariants$1,
					className: "inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md shadow-2xl hover:bg-white/10 transition-colors",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "relative flex h-3 w-3",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "animate-ping absolute inline-flex h-full w-full rounded-full bg-brand-cyan opacity-75" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "relative inline-flex rounded-full h-3 w-3 bg-brand-cyan" })]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "text-sm sm:text-base font-semibold text-slate-200 tracking-wide",
						children: t("hero.available")
					})]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.h1, {
					variants: itemVariants$1,
					className: "text-6xl sm:text-7xl lg:text-9xl py-2 font-black tracking-tighter leading-[1.1] text-white",
					children: [
						t("hero.greeting"),
						" ",
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "text-gradient",
							children: "Isaac Wu"
						})
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
					variants: itemVariants$1,
					className: "text-xl sm:text-2xl lg:text-3xl text-slate-400 font-mono h-16 sm:h-20 mt-6 lg:mt-8 flex items-center justify-center",
					children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(m, {
						sequence: [
							t("hero.developer"),
							2e3,
							t("hero.problemSolver"),
							2e3,
							t("hero.uiuxEngineer"),
							2e3,
							t("hero.lifelongLearner"),
							2e3
						],
						wrapper: "span",
						speed: 40,
						repeat: Infinity,
						className: "text-white/80"
					}, i18n.language)
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					variants: itemVariants$1,
					className: "flex flex-col sm:flex-row gap-6 mt-8 w-full sm:w-auto",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("a", {
						href: "#showcases",
						className: "group relative px-10 py-4 rounded-2xl bg-slate-50 text-slate-900 font-bold tracking-wide transition-all hover:scale-[1.03] active:scale-95 w-full sm:w-auto overflow-hidden",
						children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "absolute inset-0 w-full h-full bg-gradient-to-r from-brand-cyan via-brand-purple to-brand-pink opacity-0 group-hover:opacity-20 transition-opacity" }), t("hero.viewWork")]
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("a", {
						href: "#contact",
						className: "px-10 py-4 rounded-2xl w-full sm:w-auto backdrop-blur-md bg-white/5 hover:bg-white/10 text-slate-100 font-bold border border-white/10 transition-all hover:scale-[1.03] active:scale-95",
						children: t("hero.contactMe")
					})]
				})
			]
		})]
	});
};
var variants = {
	enter: (direction) => {
		return {
			x: direction > 0 ? 1e3 : -1e3,
			opacity: 0
		};
	},
	center: {
		zIndex: 1,
		x: 0,
		opacity: 1
	},
	exit: (direction) => {
		return {
			zIndex: 0,
			x: direction < 0 ? 1e3 : -1e3,
			opacity: 0
		};
	}
};
/**
* Experimenting with distilling swipe offset and velocity into a single variable, so the
* less distance a user has swiped, the more velocity they need to register as a swipe.
* Should accomodate longer swipes and short flicks without having binary checks on
* just distance thresholds and velocity > 0.
*/
var swipeConfidenceThreshold = 1e4;
var swipePower = (offset, velocity) => {
	return Math.abs(offset) * velocity;
};
function SlideCard({ screenshots }) {
	const [[page, direction], setPage] = (0, import_react.useState)([0, 0]);
	const imageIndex = wrap(0, screenshots.length, page);
	const paginate = (newDirection) => {
		setPage([page + newDirection, newDirection]);
	};
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(AnimatePresence, {
			initial: false,
			custom: direction,
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.img, {
				loading: "lazy",
				className: "absolute inset-0 object-contain w-full h-full p-2",
				src: screenshots[imageIndex],
				custom: direction,
				variants,
				initial: "enter",
				animate: "center",
				exit: "exit",
				transition: {
					x: {
						type: "spring",
						stiffness: 300,
						damping: 30
					},
					opacity: { duration: .2 }
				},
				drag: "x",
				dragConstraints: {
					left: 0,
					right: 0
				},
				dragElastic: 1,
				onDragEnd: (_e, { offset, velocity }) => {
					const swipe = swipePower(offset.x, velocity.x);
					if (swipe < -1e4) paginate(1);
					else if (swipe > swipeConfidenceThreshold) paginate(-1);
				}
			}, page)
		}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "absolute z-10 flex items-center justify-center w-12 h-12 text-2xl text-white bg-black/30 hover:bg-brand-cyan/20 border border-white/10 hover:border-brand-cyan/50 backdrop-blur-md rounded-full cursor-pointer select-none right-4 top-1/2 -translate-y-1/2 hover:scale-110 transition-all shadow-[0_0_15px_rgba(0,0,0,0.5)] flex items-center justify-center group",
			onClick: () => paginate(1),
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "group-hover:translate-x-0.5 transition-transform",
				children: "❯"
			})
		}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
			className: "absolute z-10 flex items-center justify-center w-12 h-12 text-2xl text-white bg-black/30 hover:bg-brand-cyan/20 border border-white/10 hover:border-brand-cyan/50 backdrop-blur-md rounded-full cursor-pointer select-none left-4 top-1/2 -translate-y-1/2 hover:scale-110 transition-all shadow-[0_0_15px_rgba(0,0,0,0.5)] flex items-center justify-center group",
			onClick: () => paginate(-1),
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "group-hover:-translate-x-0.5 transition-transform",
				children: "❮"
			})
		})
	] });
}
var Showcase = ({ index, name, desc, libraries, screenshots, deployedUrl, githubUrl, role, demoUsername, demoPswd }) => {
	const techStack = libraries.split(",").map((t) => t.trim()).filter(Boolean);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
		initial: {
			opacity: 0,
			y: 50
		},
		whileInView: {
			opacity: 1,
			y: 0
		},
		transition: {
			duration: .8,
			delay: index % 2 * .2
		},
		viewport: {
			once: true,
			margin: "-50px"
		},
		className: "flex flex-col w-full h-full mx-auto glass-card rounded-[2rem] overflow-hidden group",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "relative w-full h-[300px] sm:h-[350px] shrink-0 overflow-hidden bg-slate-900 border-b border-white/5",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute inset-0 bg-gradient-to-t from-slate-900/80 via-transparent to-transparent z-10 pointer-events-none" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)(SlideCard, { screenshots }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "absolute top-4 left-4 z-20 flex gap-2",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-full bg-red-400" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-full bg-amber-400" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "w-3 h-3 rounded-full bg-green-400" })
					]
				})
			]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "flex flex-col flex-grow w-full px-8 py-8 relative -mt-6 z-20 bg-slate-900/90 backdrop-blur-xl rounded-t-[2rem] border-t border-white/10",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex flex-wrap items-center justify-between gap-4 mb-6",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", {
						className: "text-3xl font-bold tracking-tight text-white group-hover:text-brand-cyan transition-colors duration-300",
						children: name
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "flex gap-3",
						children: [githubUrl && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("a", {
							className: "w-12 h-12 flex items-center justify-center rounded-full bg-white/5 hover:bg-white/15 border border-white/10 transition-all text-slate-300 hover:text-white hover:scale-110 active:scale-95 shadow-[0_0_15px_rgba(255,255,255,0.05)]",
							href: githubUrl,
							target: "_blank",
							rel: "noreferrer",
							title: "View Source",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CodeXml, {
								size: 22,
								strokeWidth: 2
							})
						}), deployedUrl && /* @__PURE__ */ (0, import_jsx_runtime.jsx)("a", {
							className: "w-12 h-12 flex items-center justify-center rounded-full bg-brand-cyan/20 hover:bg-brand-cyan/40 border border-brand-cyan/30 transition-all text-brand-cyan hover:text-white hover:scale-110 active:scale-95 glow-cyan",
							href: deployedUrl,
							target: "_blank",
							rel: "noreferrer",
							children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ExternalLink, {
								size: 22,
								strokeWidth: 2
							})
						})]
					})]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
					className: "text-slate-300 leading-relaxed text-base flex-grow mb-6 font-light whitespace-pre-wrap",
					children: desc.split("*").map((part, i) => i === 0 ? part : /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
						className: "block mt-2 opacity-80 text-sm",
						children: ["*", part]
					}, i))
				}),
				role && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "flex items-center gap-3 mb-4 p-3 rounded-xl bg-brand-purple/10 border border-brand-purple/20",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)(UserRound, {
							size: 18,
							className: "text-brand-purple"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "font-semibold text-brand-purple text-sm",
							children: "Role"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "text-slate-200 font-medium text-sm",
							children: role
						})
					]
				}),
				demoUsername && demoPswd && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "bg-slate-950/50 p-4 rounded-xl border border-white/10 text-xs text-slate-400 font-mono flex flex-col gap-3 mb-6 shadow-inner",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex items-center gap-2 text-slate-300 mb-1",
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(KeyRound, {
								size: 16,
								className: "text-brand-pink"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "font-bold tracking-widest text-[11px] uppercase text-brand-pink/90",
								children: "Test Credentials"
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex justify-between items-center bg-black/40 px-4 py-2.5 rounded-lg border border-white/5",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "text-slate-500 font-medium tracking-wide",
									children: "User"
								}),
								" ",
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "text-brand-cyan select-all font-bold",
									children: demoUsername
								})
							]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex justify-between items-center bg-black/40 px-4 py-2.5 rounded-lg border border-white/5",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "text-slate-500 font-medium tracking-wide",
									children: "Pass"
								}),
								" ",
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
									className: "text-brand-cyan select-all font-bold",
									children: demoPswd
								})
							]
						})
					]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "flex flex-wrap gap-2 pt-6 mt-auto border-t border-white/10",
					children: techStack.map((tech) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "px-4 py-1.5 text-[11px] font-bold tracking-wider text-brand-cyan bg-brand-cyan/10 border border-brand-cyan/20 rounded-full hover:bg-brand-cyan/20 cursor-default transition-colors",
						children: tech
					}, tech))
				})
			]
		})]
	});
};
var showcaseData = [
	{
		name: "MVDB",
		desc: "A multilanguage pure front-end website enables users to see and look up movie info.",
		libraries: "Ant Design, Axios, TanStack Router, React-spring, TanStack Query, React-i18next",
		screenshots: mvdbScreenshots,
		deployedUrl: "https://spiffy-scone-79afc0.netlify.app/",
		githubUrl: "https://gitlab.com/isaac.run.dev/mvdb"
	},
	{
		name: "YouBike Live Data Tracker",
		desc: "A web-based service enables users to check live data of the bicycle-sharing system (YouBike) in Taipei city",
		libraries: "Vite, ReactJS, TailwindCSS, Shadcn/ui, React Query, Axios",
		screenshots: youbikeScreenshots,
		deployedUrl: "https://youbike-tan.vercel.app/",
		githubUrl: "https://github.com/isaacrundev/Youbike"
	},
	{
		name: "Diet or Die",
		desc: "A website for recording diet by bringing your own data or importing from OpenFoodFacts API *This project is built by using serverless database, you might see errors when sending the first request. Please try again later.",
		libraries: "NextJS, ReactJS, NextAuthJS, Prisma, PostgreSQL, TailwindCSS, Shadcn/ui, React Hook Form, Zod, BcryptJS",
		screenshots: dodScreenshots,
		deployedUrl: "https://dod-nextjs.vercel.app/",
		githubUrl: "https://github.com/isaacrundev/dod-nextjs",
		demoUsername: "aloha@qmail.com",
		demoPswd: "111111"
	},
	{
		name: "FoxBook",
		desc: "A CRUD MERN stack website for sharing posts among registered users",
		libraries: "MongoDB, ExpressJS, ReactJS, NodeJS, Mongoose, TailwindCSS, Flowbite, React Hook Form, React-router-dom, Zod, BcryptJS. *This project is built by using serverless database, you might see errors when sending the first request. Please try again later.",
		screenshots: foxBookScreenshots,
		deployedUrl: "https://nodejs-final-client.vercel.app/",
		githubUrl: "https://github.com/isaacrundev/nodejs-final"
	},
	{
		name: "Whistler App",
		desc: "An anonymous system for reporting inappropriate behaviors to admins. For both reporters and admins",
		libraries: "React, TypeScript, Redux(Redux-toolkit), React Router DOM, Material-UI, React Query, Axios, React-beautiful-dnd",
		screenshots: whistlerAppScreenshots,
		deployedUrl: "https://dev-whistler-app.vercel.app/login",
		githubUrl: "https://github.com/WMAD-0522-Final-Project/whistleblower-app-backend",
		role: "Frontend developer"
	},
	{
		name: "Card Game",
		desc: "Get a new deck of cards every single game. 100% no cheating guaranteed",
		libraries: "HTML, CSS, JavaScript, Bootstrap, jQuery, GSAP",
		screenshots: cardGameScreenshots,
		deployedUrl: "https://shiny-haupia-090eab.netlify.app/",
		githubUrl: "https://github.com/isaacrundev/js_final"
	}
];
var getDescKey = (name) => {
	switch (name) {
		case "MVDB": return "mvdbDesc";
		case "YouBike Live Data Tracker": return "youbikeDesc";
		case "Diet or Die": return "dodDesc";
		case "FoxBook": return "foxbookDesc";
		case "Whistler App": return "whistlerDesc";
		case "Card Game": return "cardgameDesc";
		default: return "";
	}
};
var Showcases = () => {
	const { t } = useTranslation();
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		id: "showcases",
		className: "flex flex-col items-center py-16 md:py-20 relative scroll-mt-24",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute top-1/4 right-0 w-[500px] h-[500px] bg-brand-cyan/10 rounded-full blur-[120px] -z-10 pointer-events-none" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute bottom-0 left-1/4 w-[600px] h-[600px] bg-brand-purple/10 rounded-full blur-[150px] -z-10 pointer-events-none" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "flex flex-col w-full max-w-7xl px-6 sm:px-12",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					initial: {
						opacity: 0,
						y: 20
					},
					whileInView: {
						opacity: 1,
						y: 0
					},
					viewport: { once: true },
					transition: { duration: .8 },
					className: "flex flex-col items-center mb-12 text-center",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "text-brand-cyan font-mono font-bold tracking-widest uppercase mb-4 text-sm",
						children: t("showcases.subtitle")
					}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
						className: "text-5xl md:text-6xl font-black tracking-tight text-white relative",
						children: [
							t("showcases.title1"),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "text-gradient",
								children: t("showcases.title2")
							}),
							/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute -bottom-4 left-1/4 right-1/4 h-1 bg-gradient-to-r from-transparent via-brand-purple to-transparent opacity-50" })
						]
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
					className: "grid grid-cols-1 lg:grid-cols-2 gap-8 xl:gap-14 w-full items-stretch justify-items-center",
					children: showcaseData.map((item, i) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Showcase, {
						index: i,
						name: item.name,
						desc: t(`showcases.${getDescKey(item.name)}`, item.desc),
						libraries: item.libraries,
						screenshots: item.screenshots,
						deployedUrl: item.deployedUrl,
						githubUrl: item.githubUrl,
						role: item.role ? t("showcases.role") : void 0,
						demoUsername: item.demoUsername,
						demoPswd: item.demoPswd
					}, item.name))
				})]
			})
		]
	});
};
var containerVariants = {
	hidden: { opacity: 0 },
	visible: {
		opacity: 1,
		transition: { staggerChildren: .15 }
	}
};
var itemVariants = {
	hidden: {
		opacity: 0,
		y: 40,
		filter: "blur(8px)"
	},
	visible: {
		opacity: 1,
		y: 0,
		filter: "blur(0px)",
		transition: {
			duration: .8,
			ease: "easeOut"
		}
	}
};
var About = () => {
	const { t } = useTranslation();
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("section", {
		id: "about",
		className: "py-20 relative overflow-hidden flex justify-center scroll-mt-24",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			className: "w-full max-w-7xl px-8 flex flex-col items-center",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(motion.div, {
				initial: {
					opacity: 0,
					y: 20
				},
				whileInView: {
					opacity: 1,
					y: 0
				},
				transition: { duration: .8 },
				viewport: {
					once: true,
					margin: "-100px"
				},
				className: "text-center mb-12",
				children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("h2", {
					className: "text-5xl md:text-6xl font-black tracking-tight text-white inline-block relative",
					children: [
						t("about.title1"),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "text-gradient",
							children: t("about.title2")
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute -bottom-4 left-1/4 right-1/4 h-1 bg-gradient-to-r from-transparent via-brand-cyan to-transparent opacity-50" })
					]
				})
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
				variants: containerVariants,
				initial: "hidden",
				whileInView: "visible",
				viewport: {
					once: true,
					margin: "-100px"
				},
				className: "grid grid-cols-1 lg:grid-cols-12 gap-8 w-full",
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					variants: itemVariants,
					className: "lg:col-span-5 relative group rounded-[2.5rem] p-2 glass-card overflow-hidden",
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute inset-0 bg-gradient-to-br from-brand-cyan/20 to-brand-purple/20 opacity-0 group-hover:opacity-100 transition-opacity duration-700 blur-[2px]" }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("img", {
						className: "relative object-cover w-full h-[500px] rounded-[2.2rem] transition-transform duration-700 group-hover:scale-[1.02] filter group-hover:contrast-110",
						src: _14C60149_3736_4BE8_86D4_AAF1BDA7F2AF_1_201_a_default,
						alt: "Isaac Wu"
					})]
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(motion.div, {
					variants: itemVariants,
					className: "lg:col-span-7 flex flex-col justify-center p-7 lg:p-10 glass-card rounded-[2.5rem] relative overflow-hidden",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute -right-32 -top-32 w-64 h-64 bg-brand-cyan/10 rounded-full blur-[80px]" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "absolute -bottom-32 -left-32 w-64 h-64 bg-brand-pink/10 rounded-full blur-[80px]" }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex flex-col gap-8 text-slate-300 text-xl leading-relaxed font-light z-10 w-full",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Trans, {
									i18nKey: "about.intro",
									components: { 1: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { className: "text-white font-bold" }) }
								}) }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: t("about.story") }),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "bg-white/5 p-6 rounded-2xl border border-white/5 shadow-inner",
									children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
										className: "italic text-slate-200",
										children: [
											t("about.quote1"),
											/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
												className: "text-gradient font-bold not-italic",
												children: t("about.quote2")
											}),
											t("about.quote3")
										]
									})
								})
							]
						})
					]
				})]
			})]
		})
	});
};
var menuItem = [
	"about",
	"showcases",
	"contact"
];
function App() {
	const [scrollPosition, setScrollPosition] = (0, import_react.useState)(0);
	const { t } = useTranslation();
	const BackToTopVisibility = () => {
		const position = window.scrollY;
		setScrollPosition(position);
	};
	(0, import_react.useEffect)(() => {
		window.addEventListener("scroll", BackToTopVisibility);
		return () => window.removeEventListener("scroll", BackToTopVisibility);
	}, []);
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
		/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Header, { children: [menuItem.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MenuButton, {
			text: t(`nav.${item}`),
			linkTo: "#" + item
		}, item)), /* @__PURE__ */ (0, import_jsx_runtime.jsx)(LanguageSwitcher, {})] }),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Hero, {}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(About, {}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Showcases, {}),
		/* @__PURE__ */ (0, import_jsx_runtime.jsx)(Contact, {}),
		scrollPosition > 64 && /* @__PURE__ */ (0, import_jsx_runtime.jsx)(BackToTop, {})
	] });
}
//#endregion
export { App as component };
